...
void turn_on_robot::Cliff_Callback(const sensor_msgs::Range &distance)//地检
{
  if(!cliff_switch)
    {
  ...
  if(distance.range > cliff_threshold)
    {
	cliff_detected = true;
	return;
    }
...
void turn_on_robot::Cmd_Vel_Callback(const skyee_msg::ControlCommand &twist_aux)
{
  float linear_x = twist_aux.velocity.linear.x;
  float angular_z = twist_aux.velocity.angular.z;
  if (cliff_switch && cliff_detected && linear_x > 0)
  {
    linear_x = 0;
  }
  ...
  WriteMotionFrameToMcu(linear_x, twist_aux.velocity.linear.y, angular_z);
}

uint8_t turn_on_robot::BuildChassisCommandTxByte() const
{
  uint8_t cmd = 0;
  if (clean_system_state_.clean_model.data)
  {
    cmd |= CHASSIS_CMD_BIT_CLEAN_MODEL;
  }
  if (clean_system_state_.switch_control.data)
  {
    cmd |= CHASSIS_CMD_BIT_SWITCH_CONTROL;
  }
  if (clean_system_state_.beside_brush.data)
  {
    cmd |= CHASSIS_CMD_BIT_BESIDE_BRUSH;
  }
  if (clean_system_state_.middle_brush.data)
  {
    cmd |= CHASSIS_CMD_BIT_MIDDLE_BRUSH;//中刷，bit为1
  }
  if (clean_system_state_.pushbeam.data)
  {
    cmd |= CHASSIS_CMD_BIT_PUSHBEAM;//推杆
  }
  if (clean_system_state_.fan.data)
  {
    cmd |= CHASSIS_CMD_BIT_FAN;
  }
  if (clean_system_state_.water_pump.data)
  {
    cmd |= CHASSIS_CMD_BIT_WATER_PUMP;
  }
  if (clean_system_state_.vibrating_dust.data)
  {
    cmd |= CHASSIS_CMD_BIT_VIBRATING_DUST;
  }
  return cmd;
}

void turn_on_robot::WriteMotionFrameToMcu(float linear_x, float linear_y, float angular_z)
{
  short transition = 0;

  Send_Data.tx[0] = FRAME_HEADER;
  Send_Data.tx[1] = BuildChassisCommandTxByte();
  Send_Data.tx[2] = 0;

  transition = static_cast<short>(linear_x * 1000);
  Send_Data.tx[3] = transition >> 8;
  Send_Data.tx[4] = transition;

  transition = static_cast<short>(linear_y * 1000);
  Send_Data.tx[5] = transition >> 8;
  Send_Data.tx[6] = transition;

  transition = static_cast<short>(angular_z * 1000);
...
功能: 发布电池状态
***************************************/
int turn_on_robot::VoltageToPercent(float voltage) const
{
    // 功能：把下位机上报的电池电压（单位：V）换算成整数电量百分比
    // 输入：voltage，来自串口解析后的 Power_voltage
    // 输出：10 / 30 / 50 / 70 / 100，发布到 /robot_state/battery_percent
    // 说明：当前是分段阈值映射，不是精确线性换算，阈值需按实际电池标定
    ...
    if (battery_voltage_v < kVoltageThresholdVeryLow)
    {
        battery_percent = 10;
    }
    else if (battery_voltage_v < kVoltageThresholdLow)
    {
        battery_percent = 30;
    }
    else if (battery_voltage_v < kVoltageThresholdMedium)
    {
        battery_percent = 50;
    }
    else if (battery_voltage_v < kVoltageThresholdHigh)
    {
        battery_percent = 70;
    }
    else
...
int turn_on_robot::GetBatteryStateFromChassis() const
{   //20260526 BY HPP
    // 底盘状态字节：由串口 rx[1] 解析，缓存在 Receive_Data.Flag_Stop 中
    // bit0 (CHASSIS_STATUS_BIT_BATTERY_STATE)：1=充电中，0=未充电
    // 返回值与 path_planner 中 BatteryState 枚举对齐：
    //   0 -> BATTERYSTATE_NORMAL（正常）
    //   1 -> BATTERYSTATE_STATION_CHARGING（充电桩充电）


    //     enum BatteryState
    // {
    //   BATTERYSTATE_NORMAL = 0,//正常
    //   BATTERYSTATE_STATION_CHARGING = 1,//充电桩充电
    //   BATTERYSTATE_MANUAL_CHARGING = 2,//手动充电
    // };
    const uint8_t chassis_status_byte = Receive_Data.Flag_Stop;
    const uint8_t battery_state_mask = CHASSIS_STATUS_BIT_BATTERY_STATE;
    const uint8_t battery_state_raw = chassis_status_byte & battery_state_mask;//与运算用battert_state_raw表示充电状态
    const bool is_charging = (battery_state_raw != 0);
...
bool turn_on_robot::IsEmergencyStopPressed() const
{   //20260526 BY HPP
    // 底盘状态字节：由串口 rx[1] 解析，缓存在 Receive_Data.Flag_Stop 中
    // bit1 (CHASSIS_STATUS_BIT_EMERGENCY_STOP)：1=急停按下，0=急停释放
    // 充电状态由 GetBatteryStateFromChassis() 单独处理
    const uint8_t chassis_status_byte = Receive_Data.Flag_Stop;
    const uint8_t emergency_stop_mask = CHASSIS_STATUS_BIT_EMERGENCY_STOP;
    const uint8_t emergency_stop_raw = chassis_status_byte & emergency_stop_mask;
    const bool emergency_stop_pressed = (emergency_stop_raw != 0);
...
void turn_on_robot::Publish_Battery()
{
    std_msgs::Int32 battery_percent;
    std_msgs::Int8 battery_state;
    std_msgs::Bool scram_button;
    battery_percent.data = VoltageToPercent(Power_voltage);
    battery_state.data = GetBatteryStateFromChassis();
    scram_button.data = IsEmergencyStopPressed();
    battery_percent_publisher.publish(battery_percent);
    battery_state_publisher.publish(battery_state);
    scram_button_publisher.publish(scram_button);

    skyee_msg::RobotStatusInfo robot_status_msgs = BuildEvaRobotStatusInfo();
    eva_robot_status.publish(robot_status_msgs);
}

/**************************************
Date: December 25, 2025
Function: Build eva/robot_state message for App (compatible with skyee 5-byte status array)
功能: 组包 eva/robot_status，与 http_server 旧协议对齐：status[0] 位图，status[1] 电量
***************************************/
skyee_msg::RobotStatusInfo turn_on_robot::BuildEvaRobotStatusInfo() const
{
    skyee_msg::RobotStatusInfo robot_status_msgs;
    robot_status_msgs.equipment_ID = 0;
    robot_status_msgs.status.resize(5);

    uint8_t status_byte0 = 0;
    if (GetBatteryStateFromChassis() == 1)
    {
        status_byte0 |= 0x01;
    }
    if (IsEmergencyStopPressed())
    ...
    robot_status_msgs.status[0] = static_cast<int16_t>(status_byte0);
    robot_status_msgs.status[1] = static_cast<int16_t>(VoltageToPercent(Power_voltage));
    robot_status_msgs.status[2] = 0;
    robot_status_msgs.status[3] = 0;
    robot_status_msgs.status[4] = 0;
    return robot_status_msgs;
}

void turn_on_robot::ApplyCleanSystemCommand(const skyee_msg::clean_system &cmd)
{
  clean_system_state_ = cmd;
  robot_clean_model_ = cmd.clean_model.data;
  clean_system_state_.clean_model.data = robot_clean_model_;

  if (cmd.switch_control.data)
  {
    clean_system_state_.beside_brush.data = true;
    clean_system_state_.middle_brush.data = true;
    clean_system_state_.pushbeam.data = true;
    clean_system_state_.fan.data = true;
    clean_system_state_.vibrating_dust.data = false;
    clean_system_state_.baffle.data = false;
  }

  clean_system_state_.water_pump.data = cmd.water_pump.data;
  clean_system_state_.vibrating_dust.data = cmd.switch_control.data ? false : cmd.vibrating_dust.data;
  clean_system_state_.baffle.data = cmd.switch_control.data ? false : cmd.baffle.data;
  clean_system_state_.lamp.data = cmd.lamp.data;
}

void turn_on_robot::ApplyCleanSystemHardware()
{
  WriteMotionFrameToMcu(0.0f, 0.0f, 0.0f);
  ROS_INFO_THROTTLE(5.0,
                    "tx[1]=0x%02X switch=%d lamp=%d fan=%d beside=%d middle=%d pushbeam=%d",
                    BuildChassisCommandTxByte(),
                    clean_system_state_.switch_control.data,
                    clean_system_state_.lamp.data,
                    clean_system_state_.fan.data,
                    clean_system_state_.beside_brush.data,
                    clean_system_state_.middle_brush.data,
                    clean_system_state_.pushbeam.data);
}

void turn_on_robot::PublishCleanSystemAndroid()
{
  clean_system_status_publisher_.publish(clean_system_state_);
}

void turn_on_robot::gpioWriteCallback(const skyee_msg::clean_system::ConstPtr &msg)
{
  if (!msg)
  {
    return;
  }
  ApplyCleanSystemCommand(*msg);
  ApplyCleanSystemHardware();
  PublishCleanSystemAndroid();
}
  ...
  */
  ...
  if (Receive_Data.Frame_Header == FRAME_HEADER ) //Judge the frame header //判断帧头
  {
    ...
    { 
      ...
      {
        Receive_Data.Flag_Stop = Receive_Data.rx[1]; // chassis status: bit0=battery, bit1=emergency stop
...
void turn_on_robot::Control()
{
  ...
  while(ros::ok())
  {
                                   ...
                                   //通过串口读取并校验下位机发送过来的数据，然后数据转换为国际单位
    {
      ...
      Publish_Battery();//add by HPP
      // Publish_Eva_Robot_Status(); //ADD BY HPP
...
功能: 构造函数, 只执行一次，用于初始化
***************************************/
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0),cliff_detected(false)
{
  //Clear the data
  //清空数据
  ...
  private_nh.param<bool>("cliff_switch",cliff_switch,false);
 
  private_nh.param<float>("cliff_threshold",cliff_threshold,3.0);//地检
  private_nh.param<bool>("clean_model", robot_clean_model_, false);

  voltage_publisher = n.advertise<std_msgs::Float32>("PowerVoltage", 10);  //创建电池电压话题发布者
  battery_percent_publisher = n.advertise<std_msgs::Int32>("robot_state/battery_percent", 10, false);
  battery_state_publisher = n.advertise<std_msgs::Int8>("robot_state/battery_state", 1, false);
  eva_robot_status = n.advertise<skyee_msg::RobotStatusInfo>("eva/robot_state", 10,false);  //创建EVB状态话题发布者
  scram_button_publisher = n.advertise<std_msgs::Bool>("robot_state/scram_button", 1, false);
  clean_system_status_publisher_ = n.advertise<skyee_msg::clean_system>("/robot_state/clean_system_android", 1, false);

  odom_publisher    = n.advertise<nav_msgs::Odometry>("odom", 50); //创建里程计话题发布者
  imu_publisher     = n.advertise<sensor_msgs::Imu>("imu", 20);

  // /clean/cmd/control: App下行命令(订阅); /robot_state/clean_system_android: 状态回显(发布)
  Cmd_Vel_Sub     = n.subscribe("obst_cmd_vel",     100, &turn_on_robot::Cmd_Vel_Callback, this); 
  gpio_write_sub_ = n.subscribe("/clean/cmd/control", 1, &turn_on_robot::gpioWriteCallback, this);
  Cliff_Sub = n.subscribe("laser_ld", 300, &turn_on_robot::Cliff_Callback,this);
...
{
  clean_system_state_.switch_control.data = false;
  clean_system_state_.beside_brush.data = false;
  clean_system_state_.middle_brush.data = false;
  clean_system_state_.pushbeam.data = false;
  clean_system_state_.fan.data = false;
  clean_system_state_.water_pump.data = false;
  clean_system_state_.vibrating_dust.data = false;
  clean_system_state_.baffle.data = false;
  clean_system_state_.lamp.data = false;
